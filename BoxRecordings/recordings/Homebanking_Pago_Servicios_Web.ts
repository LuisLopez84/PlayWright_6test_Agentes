...
test('test', async ({ page }) => {
  ...
  await page.getByLabel('Selecciona el Servicio').selectOption('SRV002');
  await page.getByRole('spinbutton', { name: 'Monto a Pagar' }).click();
  await page.getByRole('spinbutton', { name: 'Monto a Pagar' }).fill('1200');
  await page.getByLabel('Cuenta a Debitar').selectOption('ACC003');
