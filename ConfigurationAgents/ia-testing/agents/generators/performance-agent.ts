...
export function generatePerformance(name: string, url: string) {
  ...
  // Risk 3: no sobrescribir si ya fue editado manualmente
  if (fs.existsSync(file)) {
    console.log(`⚠️ Performance spec ya existe, omitiendo: ${file}`);
    return;
  }
...
const TARGET_URL = process.env.BASE_URL || '${url}';
