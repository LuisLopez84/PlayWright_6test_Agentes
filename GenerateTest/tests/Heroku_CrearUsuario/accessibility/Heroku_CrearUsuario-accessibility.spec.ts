...
test.describe('Accessibility: Heroku_CrearUsuario', () => {
  ...
  test('axe scan — sin violaciones críticas', async ({ page }) => {
    ...
    console.log(`♿ Accessibility scan — Heroku_CrearUsuario:`);
    ...
    expect(critical.length, `${critical.length} violaciones críticas en Heroku_CrearUsuario`).toBe(0);
    expect(serious.length,  `${serious.length} violaciones graves en Heroku_CrearUsuario`).toBe(0);
  });
