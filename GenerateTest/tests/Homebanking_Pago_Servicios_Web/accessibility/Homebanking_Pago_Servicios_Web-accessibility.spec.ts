...
test.describe('Accessibility: Homebanking_Pago_Servicios_Web', () => {
  ...
  test('axe scan — sin violaciones críticas', async ({ page }) => {
    ...
    console.log(`♿ Accessibility scan — Homebanking_Pago_Servicios_Web:`);
    ...
    expect(critical.length, `${critical.length} violaciones críticas en Homebanking_Pago_Servicios_Web`).toBe(0);
    expect(serious.length,  `${serious.length} violaciones graves en Homebanking_Pago_Servicios_Web`).toBe(0);
  });
