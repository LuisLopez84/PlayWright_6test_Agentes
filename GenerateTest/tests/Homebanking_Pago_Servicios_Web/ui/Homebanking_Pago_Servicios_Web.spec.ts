...
test('Homebanking_Pago_Servicios_Web', async ({ page }) => {
  await smartGoto(page, 'Homebanking_Pago_Servicios_Web');
  ...
  await smartSelect(page, `page.getByLabel('Selecciona el Servicio')`, 'SRV002');
  await smartClick(page, `page.getByRole('spinbutton', { name: 'Monto a Pagar' })`);
  await smartFill(page, `page.getByRole('spinbutton', { name: 'Monto a Pagar' })`, '1200');
  await page.waitForTimeout(FILL_WAIT_MS);
