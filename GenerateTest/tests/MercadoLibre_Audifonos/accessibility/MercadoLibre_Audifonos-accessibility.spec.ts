...
test.describe('Accessibility: MercadoLibre_Audifonos', () => {
  ...
  test('axe scan — sin violaciones críticas', async ({ page }) => {
    ...
    console.log(`♿ Accessibility scan — MercadoLibre_Audifonos:`);
    ...
    expect(critical.length, `${critical.length} violaciones críticas en MercadoLibre_Audifonos`).toBe(0);
    expect(serious.length,  `${serious.length} violaciones graves en MercadoLibre_Audifonos`).toBe(0);
  });
