...
/**
 * Visual regression — MercadoLibre_Audifonos
...
const TARGET_URL = process.env.BASE_URL || 'https://www.mercadolibre.com.co';
...
test.describe('Visual regression: MercadoLibre_Audifonos', () => {
  ...
  test('screenshot — desktop 1280×720', async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot('MercadoLibre_Audifonos-desktop.png', {
      fullPage:           true,
  ...
  test('screenshot — mobile 375×812', async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot('MercadoLibre_Audifonos-mobile.png', {
      fullPage:           true,
      maxDiffPixelRatio:  VISUAL_DIFF_RATIO,
      animations:         'disabled',
    });
