...
test.describe('Accessibility: Homebanking_PrestamosTesting', () => {
  ...
  test('axe scan — sin violaciones críticas', async ({ page }) => {
    ...
    console.log(`♿ Accessibility scan — Homebanking_PrestamosTesting:`);
    ...
    expect(critical.length, `${critical.length} violaciones críticas en Homebanking_PrestamosTesting`).toBe(0);
    expect(serious.length,  `${serious.length} violaciones graves en Homebanking_PrestamosTesting`).toBe(0);
  });
