...
test.describe('Accessibility: Homebanking_Tarjeta_Virtual', () => {
  ...
  test('axe scan — sin violaciones críticas', async ({ page }) => {
    ...
    console.log(`♿ Accessibility scan — Homebanking_Tarjeta_Virtual:`);
    ...
    expect(critical.length, `${critical.length} violaciones críticas en Homebanking_Tarjeta_Virtual`).toBe(0);
    expect(serious.length,  `${serious.length} violaciones graves en Homebanking_Tarjeta_Virtual`).toBe(0);
  });
