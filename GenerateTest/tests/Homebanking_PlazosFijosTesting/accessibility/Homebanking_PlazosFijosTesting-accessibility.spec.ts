...
test.describe('Accessibility: Homebanking_PlazosFijosTesting', () => {
  ...
  test('axe scan — sin violaciones críticas', async ({ page }) => {
    ...
    console.log(`♿ Accessibility scan — Homebanking_PlazosFijosTesting:`);
    ...
    expect(critical.length, `${critical.length} violaciones críticas en Homebanking_PlazosFijosTesting`).toBe(0);
    expect(serious.length,  `${serious.length} violaciones graves en Homebanking_PlazosFijosTesting`).toBe(0);
  });
