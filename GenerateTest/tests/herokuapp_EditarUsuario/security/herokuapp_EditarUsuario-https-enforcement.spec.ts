...
test.describe('HTTPS enforcement for herokuapp_EditarUsuario', () => {
  ...
  test('Redirect HTTP → HTTPS', async ({ request }) => {
    const httpUrl = 'https://thinking-tester-contact-list.herokuapp.com'.replace('https://', 'http://');
    try {
  ...
  test('HSTS header presente', async ({ request }) => {
    ...
    if (true) {
      ...
      if (!hsts) {
        console.warn('⚠️ Strict-Transport-Security no presente en https://thinking-tester-contact-list.herokuapp.com');
      }
      // No es fatal — no todos los sitios lo implementan, pero se reporta
      expect(true).toBeTruthy();
    } else {
      // Si no es HTTPS, el test es informativo
      console.warn('⚠️ https://thinking-tester-contact-list.herokuapp.com no usa HTTPS');
      expect(true).toBeTruthy();
    }
  });
