...
test.describe('Brute force protection for herokuapp_EditarUsuario', () => {

  test('Protección contra brute force via API', async ({ request }) => {
    const loginEndpoint = 'https://thinking-tester-contact-list.herokuapp.com/login';
    let lastStatus = 0;

    // Enviar 10 intentos fallidos consecutivos
    for (let i = 0; i < 10; i++) {
      const res = await request.post(loginEndpoint, {
        data: { username: 'Email', password: `wrong_password_${i}` }
      });
  ...
  test('Protección contra brute force via UI', async ({ page }) => {
    ...
    console.log('ℹ️ Brute force UI test: app sin formulario de login detectado (herokuapp_EditarUsuario)');
