...
test.describe('Accessibility: Shein_Audifonos', () => {
  ...
  test('axe scan — sin violaciones críticas', async ({ page }) => {
    ...
    console.log(`♿ Accessibility scan — Shein_Audifonos:`);
    ...
    expect(critical.length, `${critical.length} violaciones críticas en Shein_Audifonos`).toBe(0);
    expect(serious.length,  `${serious.length} violaciones graves en Shein_Audifonos`).toBe(0);
  });
