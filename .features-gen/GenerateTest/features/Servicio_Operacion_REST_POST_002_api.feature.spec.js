// Generated from: GenerateTest\features\Servicio_Operacion_REST_POST_002_api.feature
import { test } from "playwright-bdd";

test.describe('API REST POST - Servicio Operacion REST POST 002', () => {
  ...
  test('Servicio Operacion REST POST 002 - Petición exitosa', async ({ When, Then, request }) => { 
    await When('ejecuto la petición API configurada', null, { request }); 
    await Then('la respuesta debe tener un estado de éxito 2xx'); 
  });

  test('Servicio Operacion REST POST 002 - Error técnico del servidor', async ({ When, Then, request }) => { 
  ...
  test('Servicio Operacion REST POST 002 - Error por datos inválidos', async ({ When, Then, request }) => { 
...
test.use({
  ...
  $uri: [({}, use) => use('GenerateTest\\features\\Servicio_Operacion_REST_POST_002_api.feature'), { scope: 'test', box: true }],
