/**
 * Parchea los brute force specs de Homebanking.
 * Solo realiza 1 intento de login fallido (no 3) para evitar timeout.
 * Ejecutar con: node scripts/patch-homebanking-bruteforce.mjs
 */
...
const BASE_URL = 'https://homebanking-demo-tests.netlify.app';
const LOGIN_PATH = '/login';

const HOMEBANKING_SUITES = [
  'Homebanking_Transf',
  'Homebankink_PagoServicios',
  'Homebankink_PlazosFijos',
  'Homebankink_Prestamos',
  'Homebankink_TarjetaVirtual',
];
...
let count = 0;
for (const name of HOMEBANKING_SUITES) {
  ...
  const filePath = path.join(secDir, `${name}-authentication-bruteforce.spec.ts`);
  fs.writeFileSync(filePath, generateBruteforceSpec(name));
  console.log(`✅ ${name} — brute force spec actualizado`);
  count++;
}

console.log(`\n🎉 Listo: ${count} suites Homebanking actualizadas`);
